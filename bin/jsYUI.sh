...
# Shell interface to Yahoo YUI javascript compressor.
#
...
printhelp() {
	...
	echo "size of Javascript files located under FILEx or"
...
process_entries() {
	msg_info "Minifying ${TERM_YELLOW}Javascript${TERM_RESET} files in ${TERM_LGREEN}${1}${TERM_RESET}"
	if [ -f "${1}" ]; then
		process_file "${1}"
	elif [ -d "${1}" ]; then
...
process_file() {
	...
	if [ "$FAKE_IT" != "1" ]; then
		...
		# run java runtime
		if ! java_run_simple "${JAVA_CLASS}" "" ${opts} --type js "${fname}" -o "${tmp_fname}"; then
			return 1
		fi
